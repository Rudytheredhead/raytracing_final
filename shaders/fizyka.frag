...
bool rayTriangleIntersect(Ray ray, vec3 v0, vec3 v1, vec3 v2, vec3 n0, vec3 n1, vec3 n2, out float t, out vec3 normal, out float out_u, out float out_v) {
    vec3 e1 = v1 - v0;
    vec3 e2 = v2 - v0;
            ...
            while (blasIdx != -1) {
                ...
                if (b_ilosc > 0) { 
                    for (int i = 0; i < b_ilosc; i++) {
                        int triBase = tri_offset + (b_pierwszy + i) * 32;
                        ...
                        float u0 = fetchFloat(triBase+24);
                        float v0_tex = fetchFloat(triBase+25);
                        float u1 = fetchFloat(triBase+26);
                        float v1_tex = fetchFloat(triBase+27);
                        float u2 = fetchFloat(triBase+28);
                        float v2_tex = fetchFloat(triBase+29);
                        float has_tex = fetchFloat(triBase+30);
                        
                        float t_hit;
                        vec3 normal;
                        float hit_u, hit_v;
                        if (rayTriangleIntersect(local_ray, v0, v1, v2, n0, n1, n2, t_hit, normal, hit_u, hit_v)) {
                            if (t_hit < closest.t) { 
                                ...
                                vec3 base_col = inst_kolor * vec3(tri_col_x, tri_col_y, tri_col_z);
                                if (has_tex > 0.5) {
                                    float w = 1.0 - hit_u - hit_v;
                                    vec2 final_uv = w * vec2(u0, v0_tex) + hit_u * vec2(u1, v1_tex) + hit_v * vec2(u2, v2_tex);
                                    // Odwracanie V wylaczone - model pochodzi z ImageToStl.com
                                    vec3 tex_col = texture(diffuseMap, final_uv).rgb;
                                    base_col *= tex_col;
                                }
                                closest.kolor = base_col;
