        ...
        if (typ == "mtllib") {
            ...
            if (mtl.is_open()) {
                ...
                while (std::getline(mtl, mtl_line)) {
                    ...
                    } else if (mtyp == "map_Kd" && !current_mat.empty()) {
                        std::string tex_map; miss >> tex_map;
                        materials[current_mat].texture_map = tex_map;
                    }
                }
            } else {
            ...
            while (iss >> part) {
                ...
                if (pos1 != std::string::npos) {
                    ...
                    std::string uv_idx_str = part.substr(pos1 + 1, pos2 - pos1 - 1);
                    if (!uv_idx_str.empty()) {
                        int uv_idx = std::stoi(uv_idx_str);
                        if (uv_idx < 0) uv_idx = uvs.size() + uv_idx + 1;
                        face_uvs.push_back(uv_idx - 1);
                    }
            ...
            bool has_uvs = (face_uvs.size() == face_vertices.size());
            for (size_t i = 1; i + 1 < face_vertices.size(); ++i) {
                if (face_vertices[0] < vertices.size() && face_vertices[i] < vertices.size() && face_vertices[i+1] < vertices.size()) {
                    ...
                    float u0 = 0.0f, v0_tex = 0.0f;
                    float u1 = 0.0f, v1_tex = 0.0f;
                    float u2 = 0.0f, v2_tex = 0.0f;
                    if (has_uvs && face_uvs[0] < uvs.size() && face_uvs[i] < uvs.size() && face_uvs[i+1] < uvs.size()) {
                        u0 = uvs[face_uvs[0]].first; v0_tex = uvs[face_uvs[0]].second;
                        u1 = uvs[face_uvs[i]].first; v1_tex = uvs[face_uvs[i]].second;
                        u2 = uvs[face_uvs[i+1]].first; v2_tex = uvs[face_uvs[i+1]].second;
                    }
                    ...
                    if (active_material) {
                        ...
                        if (!active_material->texture_map.empty()) {
                            has_tex = 1;
                        }
                    }
                    ...
                    trojkaty_.emplace_back(
                        ...
                        v0, v1, v2, 
                        n0, n1, n2,
                        u0, v0_tex, u1, v1_tex, u2, v2_tex, has_tex,
                        tri_lustrzanosc, 
                        tri_metalicznosc, 
                        max_emisja
                    );
