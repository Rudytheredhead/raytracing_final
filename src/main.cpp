...
int main() {
    ...
    if (!shader_obliczenia.loadFromFile("shaders/fizyka.frag", sf::Shader::Fragment)) {
        std::cerr << "Blad ladowania shadera (fizyka)!" << std::endl;
        return -1;
    }

    sf::Shader shader_post;
    if (!shader_post.loadFromFile("shaders/post_process.frag", sf::Shader::Fragment)) {
    ...
    if (!wczytaj_scene("parametry_wejsciowe.json", obiekty, zrodla_swiatla, liczba_rdzeni, globalna_tekstura)) {
        std::cerr << "Blad przy wczytywaniu sceny!" << std::endl;
        return 1;
    }
    ...
    for(auto inst : instancje_sceny) {
        ...
        if (model_to_tri_offset.find(m) == model_to_tri_offset.end()) {
            ...
            current_tri += m->trojkaty_.size() * 32;
        }
    }
    ...
    for(auto inst : instancje_sceny) {
        ...
        if (!packed_tri[m]) {
            ...
            for(const auto& t : m->trojkaty_) {
                ...
                push_float(t.get_u(0)); push_float(t.get_v(0)); push_float(t.get_u(1)); push_float(t.get_v(1));
                push_float(t.get_u(2)); push_float(t.get_v(2)); push_float((float)t.has_texture()); push_float(0.0f);
            }
        }
    }
    ...
    if (!globalna_tekstura.empty()) {
        if (diffuseTexture.loadFromFile(globalna_tekstura)) {
            diffuseTexture.setRepeated(true);
            shader_obliczenia.setUniform("diffuseMap", diffuseTexture);
        } else {
            std::cerr << "Ostrzezenie: Nie udalo sie wczytac tekstury " << globalna_tekstura << "!" << std::endl;
        }
    }
